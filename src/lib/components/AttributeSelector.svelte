<script>
  ...
  let credentials = $derived(credentialStore.all.filter(c => c.status !== 'revoked'));
  let allCredentials = $derived(credentialStore.all);
  let selectedCredential = $derived(allCredentials.find(c => c.id === selectedCredentialId) || null);
  let selectedIsRevoked = $derived(selectedCredential?.status === 'revoked');

  $effect(() => {
    if (selectedCredential && !selectedIsRevoked) {
      const attrs = {};
  ...
  function handlePresent() {
    if (!selectedCredential || !hasSelection || selectedIsRevoked) return;
    const sharedValues = {};
...
<div class="attribute-selector">
  ...
  {:else if !selectedCredential}
    ...
    <div class="credential-list">
      {#each allCredentials as cred (cred.id)}
        <button class="credential-option" class:is-revoked={cred.status === 'revoked'} onclick={() => selectedCredentialId = cred.id}>
          <span class="option-icon">{cred.icon}</span>
          <div class="option-info">
            <span class="option-title">{cred.label}</span>
            <span class="option-issuer">{cred.issuer}</span>
            {#if cred.status === 'revoked'}
              <span class="option-revoked">{t('wallet.revoked_badge')}</span>
            {/if}
          </div>
  ...
  {:else if selectedIsRevoked}
    <div class="selection-header">
      <button class="back-btn" onclick={() => selectedCredentialId = null}>← {t('present.edit_selection')}</button>
    </div>
    <div class="revoked-warning">
      <div class="revoked-icon">🔴</div>
      <h3>{t('present.revoked_warning')}</h3>
      <p>{t('wallet.revoked_detail')}</p>
    </div>
  {:else}
    <div class="selection-header">
      <button class="back-btn" onclick={() => selectedCredentialId = null}>← {t('present.edit_selection')}</button>
...
<style>
  ...
  .credential-option.is-revoked { background: #fef2f2; border-color: #fecaca; opacity: 0.8; }
  ...
  .option-revoked { font-size: 0.7rem; color: #dc2626; font-weight: 600; }
  ...
  .revoked-warning { display: flex; flex-direction: column; align-items: center; text-align: center; padding: 2rem; background: #fef2f2; border: 1px solid #fecaca; border-radius: 16px; margin-top: 1rem; }
  .revoked-icon { font-size: 3rem; margin-bottom: 0.75rem; }
  .revoked-warning h3 { color: #dc2626; margin-bottom: 0.5rem; }
  .revoked-warning p { color: #888; font-size: 0.9rem; }

...
{:else}
  <div class="verifier">
    ...
    <div class="mode-toggle">
      <button class="mode-btn" class:active={!offlineMode && !crossDeviceMode} onclick={() => { offlineMode = false; crossDeviceMode = false; }}>
        💻 Standard
      </button>
      <button class="mode-btn cross-btn" class:active={crossDeviceMode} onclick={() => { crossDeviceMode = true; offlineMode = false; }}>
        📲 {t('verify.cross_device_toggle')}
      </button>
...
<style>
  ...
  /* Mode Toggle (Server-Status jetzt global im Header via LEDs) */
  .mode-toggle { display: flex; gap: 0.5rem; margin-bottom: 1rem; justify-content: center; flex-wrap: wrap; }
  ...
  .cross-btn.active { background: #6a1b9a; color: white; border-color: #6a1b9a; }
  ...
  .cross-device-qr .qr-image-wrapper { background: white; padding: 1.25rem; border-radius: 16px; border: 2px solid #8e24aa; box-shadow: 0 4px 20px rgba(106,27,154,0.15); margin-bottom: 0.75rem; }
  .cross-device-qr .qr-image { width: 256px; height: 256px; display: block; image-rendering: pixelated; }
